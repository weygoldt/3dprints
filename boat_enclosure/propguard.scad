...
// The slot runs through the guard_t-thick hub only (not the proud rim).  Because the canal runs DIAGONALLY out through the
// SQUARE hub's CORNER, it slices that corner into two ACUTE ~45deg PRONGS -- the sharp edges the cable chafes on as it moves.
// ROUND THE TWO PRONGS: a morphological OPEN (erode->dilate by wire_slot_corner_r) of the hub-minus-canal outline rounds
// exactly the convex prong tips tangent to BOTH the canal wall and the hub edge; the difference (outline minus its open) IS
// those tip slivers, clipped to the exit corner so only the two prongs are touched, and subtracted below.
module wire_slot_2d(ang, len) rotate([0,0,ang]) translate([len/2,0]) rect([len, wire_slot_w], rounding = wire_slot_w/2 - 0.01);
module wire_hubcut2d(ang, len) difference() { square([guard_hub_w, guard_hub_h], center = true); wire_slot_2d(ang, len); }
module guard_wire_slot(ang = wire_slot_ang) {
  len = norm([guard_hub_w/2, guard_hub_h/2]) + 5;
  ...
  if (wire_slot_corner_r > 0) {
    sx = cos(ang) >= 0 ? 1 : -1;  sy = sin(ang) >= 0 ? 1 : -1;    // which hub corner the canal exits
    translate([0,0,-eps]) linear_extrude(guard_t + 2*eps)
      intersection() {
        difference() {                                            // the prong-tip slivers = outline minus its opening
          wire_hubcut2d(ang, len);
          offset(r = +wire_slot_corner_r) offset(r = -wire_slot_corner_r) wire_hubcut2d(ang, len);
        }
        translate([sx*guard_hub_w/2, sy*guard_hub_h/2]) circle(r = 16, $fn = 64);   // only the exit corner (clear of other corners/pads)
      }
  }
}
